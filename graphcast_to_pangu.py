...
def graphcast_to_pangu(gc_config,pw_config,ds,time_gc,specob_gc,specob_pw,lev=None,vars_pl=None,verbose=True):
    ...
    for v in vars_pl:
        ...
        if k == 0:
            # initialize
            if lev == None:
                dims = np.flip(np.flip(v_ds[gc_config['var_pl'][v]].isel(batch=0,time=time_gc).to_numpy(),axis=0),axis=1).shape
            else:
                ilev = np.where(v_ds['level'].to_numpy()==lev)[0]
                if verbose: print('ilev=',ilev)
                dims = np.flip(np.flip(v_ds[gc_config['var_pl'][v]].isel(level=ilev,batch=0,time=time_gc).to_numpy(),axis=0),axis=1).shape
    ...
    for v in pw_config['var_sfc']:
        ...
        if k == 0:
            # initialize
            dims = np.flip(np.flip(v_ds[gc_config['var_sfc'][v]].isel(batch=0,time=time_gc).to_numpy(),axis=0),axis=1).shape
            tmp = tuple([Nvars_sfc]+list(dims))
            pw_sfc_on_gc = np.zeros(tmp)
        
        # flip latitude axis
        pw_sfc_on_gc[k,:,:] = np.flip(v_ds[gc_config['var_sfc'][v]].isel(batch=0,time=time_gc).to_numpy(),axis=0)
...
# load the gc source file
v_ds = xr.load_dataset(vfile)
print(v_ds)

# set up two grids: pw on gc grid and pw on pw grid (ERA5)
lat_gc = v_ds['lat'].to_numpy()
lon_gc = v_ds['lon'].to_numpy()
Nlat_gc = len(lat_gc)
Nlon_gc = len(lon_gc)
# ERA5 lat,lon grid
...
#time_gc_v = 0
#vars_pl = ['z']
#verif_pl,verif_sfc = graphcast_to_pangu(gc_config,pw_config,v_ds,time_gc_v,specob_gc,specob_pw,lev=500,vars_pl=vars_pl,verbose=False)
...
try:
    dd = v_ds['datetime'].isel(batch=0,time=time_gc).to_numpy()
    dds = np.datetime_as_string(dd, unit='h')
    rgfile = icpath+ofile+'_'+dds+'.h5'
    print('time from datetime in nc file: ',dds)
except:
    print('no datetime array in the file')
    rgfile = icpath+ofile+'.h5'

pert_pl,pert_sfc = graphcast_to_pangu(gc_config,pw_config,v_ds,time_gc,specob_gc,specob_pw)
# write the pangu IC file
#rgfile = icpath+ofile
print('writing file: ',rgfile)    
h5f = h5py.File(rgfile, 'w')
h5f.create_dataset('input_pl',data=pert_pl)
h5f.create_dataset('input_sfc',data=pert_sfc)
