"""
This version reads monthly mean data
"""
import dataclasses
...
LEVEL_DATA_PATH = "/glade/campaign/collections/rda/data/ds633.1/e5.moda.an.pl/"
SFC_DATA_PATH = "/glade/campaign/collections/rda/data/ds633.1/e5.moda.an.sfc/"
...
def open_casper_nc(codes, time):
    ...
    for code in codes:
        ...
        # e5.moda.an.sfc.228_247_100v.ll025sc.2020010100_2020120100.nc
        if isinstance(code, SingleLevelCode): # 1 month 1 data
            path = SFC_DATA_PATH + f"{year}/" + \
                f"e5.moda.an.sfc.{code.code0}_{code.id}_{code.name}.{termll025}.{year}010100_{year}120100.nc"
            # check sfc 228: lblt 100u 100v 
        elif isinstance(code, PressureLevelCode): # 1 day 1 data
            # e5.moda.an.pl.128_129_z.ll025sc.2020010100_2020120100.nc
            #LEVEL_DATA_PATH = "/glade/campaign/collections/rda/data/ds633.1/e5.moda.an.pl/"
            path = LEVEL_DATA_PATH + f"{year}/" + \
                f"e5.moda.an.pl.128_{code.id}_{code.name}.{termll025}.{year}010100_{year}120100.nc"
        else:
...
if __name__ == "__main__":
    ...
    ds = DataSource(['z500','v500'])
    res = ds[datetime.datetime(2018, 12, 1)]
    print(res)
    print(type(res))
